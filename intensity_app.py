...
def Segment():
    ...
    if raw_file is not None:
        ...
        raw_image = load_image(file_bytes)
        ...
        if extension == 'tif' and len(raw_image_ani.shape)==4:
            raw_image = raw_image_ani[0] 
            btn_clk = st.button("Show frames")
            if btn_clk:                
                st.subheader("Image frames")             
                show_ani(raw_image_ani)                            
                               
        #show_stack = st.number_input(f"Choose the frame number (0-{raw_image_ani.shape[0]-1})", min_value = 0, max_value = raw_image_ani.shape[0])
        
        st.write(raw_image.shape)
        st.image(raw_image,use_column_width=True,clamp = True)
        #processed_tiff = []
        
        med_x = st.slider("Median Blur Kernel size", min_value = 1,max_value = 100, step = 2)
        blur_median_proc = cv2.medianBlur(raw_image,med_x)
        
        bri_x = st.slider('Change Brightness',min_value = 0,max_value = 255)
        con_x = st.slider('Change Contrast',min_value = 0,max_value = 255)
        
        bri_con = apply_brightness_contrast(blur_median_proc, bri_x, con_x)
        st.image(bri_con,use_column_width=True,clamp = True)
        
        lab_img= cv2.cvtColor(bri_con, cv2.COLOR_RGB2LAB)
        l, a, b = cv2.split(lab_img)
        equ = cv2.equalizeHist(l)
        updated_lab_img1 = cv2.merge((equ,a,b))
        hist_eq_img = cv2.cvtColor(updated_lab_img1, cv2.COLOR_LAB2RGB)
        ...
        st.write("The current processed frame (after histogram equalization)")                   
        st.image(CLAHE_img,use_column_width=True,clamp = True)
        
        
        model = StarDist2D.from_pretrained('2D_versatile_fluo')
        labels, _ = model.predict_instances(normalize(CLAHE_img))
        
        seg_im = render_label(labels, img=CLAHE_img)
        ...
        get_image_download_link(rgb_image,"segmented.png")
         ...
         #    	cv2.circle(labels, (int(x), int(y)), int(r), (0, 255, 0), 1)
         #    	cv2.putText(labels, "#{}".format(label), (int(x) - 10, int(y)),
        ...
        data = list(np.unique(labels)) 
        
        df_pro = pd.DataFrame(data, columns=['label'])
        
        for frames_pro in range(0,raw_image_ani.shape[0]):
            props_pro = measure.regionprops_table(labels, intensity_image=raw_image_ani[frames_pro][:,:,0],   #markers
        ...
        labels_rgb = np.expand_dims(labels, axis=2)
